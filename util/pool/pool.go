...
// items from the pool when garbage collection happens.
package pool
...
// TakeAll removes all items from the pool, appending them to the provided
// slice (which can be nil) and returning them. The returned slice can be nil
// if the provided slice was nil and the pool was empty.
//
// This function does not free the backing storage for the pool; to do that,
// use the Clear function.
func (p *Pool[V]) TakeAll(dst []V) []V {
	ret := dst
	for i := range p.s {
		if *p.s[i].index != -1 {
...
// Peek will return the item at the given index without removing it from the
// pool. It will return false if the item has been deleted or previously taken.
...
//
// It will return ok=false and the zero value if the set is empty.
